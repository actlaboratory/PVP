...
class TestCmd_success(unittest.TestCase):
    def test_runCmdInBackground(self):
        ...
        self.assertEqual(result.logFilePath, ".\\logs\\20231201_000000_1.log")
        self.assertEqual(result.returncode, 0)
        self.assertEqual(result.exception, None)
        resultFromQueue = self.queue.get(block=True, timeout=1)
        self.assertEqual(resultFromQueue.logFilePath, ".\\logs\\20231201_000000_1.log")
...
class TestCmd_success_fileExists(unittest.TestCase):
    def test_runCmdInBackground(self):
        ...
        runner.join()
        result = runner.result()
        self.assertEqual(result.logFilePath, ".\\logs\\20231201_000000_2.log")
        self.assertEqual(result.returncode, 0)
        self.assertEqual(result.exception, None)
        resultFromQueue = self.queue.get(block=True, timeout=1)
        self.assertEqual(resultFromQueue.logFilePath, ".\\logs\\20231201_000000_2.log")
        self.assertEqual(resultFromQueue.returncode, 0)
        self.assertEqual(resultFromQueue.exception, None)
